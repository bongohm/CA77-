...
function break1clockInWithPin(pin) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var mainSheet = ss.getSheetByName("ลงเวลาพักครั้งที่1"); // "Work Time Log" sheet
  var employeeSheet = ss.getSheetByName("รายชื่อพนักงาน"); // "Employee List" sheet
  var employeeData = employeeSheet.getRange(2, 1, employeeSheet.getLastRow(), 5).getValues();
  var returnDate = '';
  var error = 'SUCCESS';
...
function sendLineNotify(message) {
  var token = "2BtjbUfOUd7FnU5Xky6IKV18Twhj7RJvVCunxBtachV"; // แทนที่ด้วย Token ของคุณ
  var options = {
    "method": "post",
    "headers": {
...
function getImageUrls() {
  var sheet = SpreadsheetApp.openById('17CftuNYzF0H6TGk-6z5p3rV5BV3j59j2IBI_w8RcIHU').getSheetByName('Sheet1'); // Adjust 'Sheet1' as needed
  var range = sheet.getRange('A:A'); // Assuming URLs are in column A; adjust as necessary
  var values = range.getValues();
  var urls = values.filter(function(value) { return value[0].trim() !== ''; }).map(function(row) { return row[0]; });
  return urls;
}
